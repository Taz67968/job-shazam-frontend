    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            setGeneratedCV({
                ...
                experience: [
                    { role: "Senior Developer", company: "Tech Corp", duration: "2020-Present", points: ["Led team of 5", "Refactored legacy code"] }
                ]
            });
                        ...
                        {generatedCV ? (
                            <div className="space-y-6 animate-fade-in">
                                <div className="p-6 bg-white text-black rounded-lg shadow-lg">
                                    ...
                                    <div className="mb-4">
                                        <h4 className="font-bold uppercase text-sm text-gray-500 mb-2">Professional Summary</h4>
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {generatedCV.skills.map((skill) => (
                                                <span key={skill} className="px-2 py-1 bg-gray-200 rounded text-xs font-medium">{skill}</span>
                                            ))}
                                        </div>
                                    </div>
                                        ...
                                        {generatedCV.experience.map((exp, i) => (
                                            <div key={i} className="mb-3">
                                                ...
                                                <ul className="list-disc list-inside mt-1">
                                                    {exp.points.map((p, j) => (
                                                        <li key={j} className="text-xs">{p}</li>
                                                    ))}
                                                </ul>
                                            </div>
                                        ))}
